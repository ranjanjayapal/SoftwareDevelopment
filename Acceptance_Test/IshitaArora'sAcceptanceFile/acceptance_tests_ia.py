...
class test_user_stories(unittest.TestCase):
    ...
    def test_upcoming_bdays(self):
        pass_lines = [line.rstrip('\n\r') for line in open("US38_PassFile.ged")]
        fail_lines = [line.rstrip('\n\r') for line in open("US38_FailFile.ged")]
        individuals_pass = parse_indi(pass_lines)
        self.assertTrue(user_stories_ia.list_upcoming_birthdays(individuals_pass))
        individuals_fail = parse_indi(fail_lines)
        self.assertFalse(user_stories_ia.list_upcoming_birthdays(individuals_fail))

    def test_recent_births(self):
        pass_lines = [line.rstrip('\n\r') for line in open("US35_PassFile.ged")]
        fail_lines = [line.rstrip('\n\r') for line in open("US35_FailFile.ged")]
        individuals_pass = parse_indi(pass_lines)
        self.assertTrue(user_stories_ia.list_recent_births(individuals_pass))
        individuals_fail = parse_indi(fail_lines)
        self.assertFalse(user_stories_ia.list_recent_births(individuals_fail))
