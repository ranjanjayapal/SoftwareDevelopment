...
class Test_file(unittest.TestCase):
    ...
    def test_recent_deaths(self):
        pass_lines = [line.rstrip('\n\r') for line in open("US36_PassFile.ged")]
        fail_lines = [line.rstrip('\n\r') for line in open("US36_FailFile.ged")]
        individuals_pass = parse_indi(pass_lines)
        self.assertTrue(user_stories_rj.list_recent_deaths(individuals_pass))
        individuals_fail = parse_indi(fail_lines)
        self.assertFalse(user_stories_rj.list_recent_deaths(individuals_fail))
    def test_deceased(self):
        pass_lines = [line.rstrip('\n\r') for line in open("US29_PassFile.ged")]
        fail_lines = [line.rstrip('\n\r') for line in open("US29_FailFile.ged")]
        individuals_pass = parse_indi(pass_lines)
        self.assertTrue(user_stories_rj.list_deceased(individuals_pass))
        individuals_fail = parse_indi(fail_lines)
        self.assertFalse(user_stories_rj.list_deceased(individuals_fail))
